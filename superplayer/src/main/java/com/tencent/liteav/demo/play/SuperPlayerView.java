...
public class SuperPlayerView extends RelativeLayout implements ITXVodPlayListener, ITXLivePlayListener {
    ...
    private String zhanwWeiTu;
    private boolean successImage = false;
    private boolean isDanmu = true;

    private OnPlayEndListener onPlayEndListener;
    private OnSuperPlayerWidthCallback onSuperPlayerWidthCallback;
    private boolean isHigth = false;
    private int WidthAndKuan = 1;

    public void setOnPlayEndListener(OnPlayEndListener onPlayEndListener) {
        this.onPlayEndListener = onPlayEndListener;
    }

    public void setOnSuperPlayerWidthCallback(OnSuperPlayerWidthCallback onSuperPlayerWidthCallback) {
        this.onSuperPlayerWidthCallback = onSuperPlayerWidthCallback;
    }
    ...
    public void setOnlyPlay(OnClickListener onClickListener) {
        mVodControllerLarge.setVisibility(GONE);
        mVodControllerSmall.setVisibility(GONE);
        mVodControllerFloat.setVisibility(GONE);
        mTXCloudVideoView.setOnClickListener(onClickListener);
    }

    public void hideControllerView() {
        mVodControllerLarge.setVisibility(GONE);
        mVodControllerSmall.setVisibility(VISIBLE);
        mVodControllerFloat.setVisibility(GONE);
    }
    ...
    public void setShowBackWhenSmall(boolean showBackWhenSmall) {
        mVodControllerSmall.setShowBack(showBackWhenSmall);
    }
    ...
    private void initVodPlayer(Context context) {
        ...
        File nomedia = new File(Environment.getExternalStorageDirectory().getPath() + "/.nomedia");
        if (!nomedia.exists())
            try {
...
//                mVodPlayer.setToken(modelWrapper.requestModel.token);
    ...
    public void playWithModel(final SuperPlayerModel modelV3, boolean isPlay, String zhanwWeiTu) {
        this.isPlay = isPlay;
        this.zhanwWeiTu = zhanwWeiTu;
        mCurrentTimeWhenPause = 0;
        SuperPlayerModelWrapper modelWrapper = new SuperPlayerModelWrapper(modelV3);
        mCurrentModelWrapper = modelWrapper;
        if (modelV3.videoId != null) { // 根据FileId播放
...
//        }
    ...
    private void playTimeShiftLiveURL(final SuperPlayerModelWrapper model) {
        ...
        try {
            int bizidNum = Integer.parseInt(bizid);
...
//            }
     ...
     * 播放器控制
     */
    public TCVodControllerBase.VodController mVodController = new TCVodControllerBase.VodController() {
        ...
        public void onRequestPlayMode(int requestPlayMode) {
            ...
            if (requestPlayMode == SuperPlayerConst.PLAYMODE_FULLSCREEN) {
                ...
                if (mLayoutParamFullScreenMode == null)
                    return;
                if (isDanmu) {
                    mVodControllerLarge.setmIvDanmukuVisibility(View.VISIBLE);
                } else {
                    mVodControllerLarge.setmIvDanmukuVisibility(View.GONE);
                }
                ...
                if (WidthAndKuan > 0) {
                    ...
                    if (onSuperPlayerWidthCallback != null) {
                        onSuperPlayerWidthCallback.showFullOrDis();
                    }
                }
...
//                mWindowParams.type = WindowManager.LayoutParams.TYPE_SYSTEM_ALERT;
        ...
        public float getCurrentPlaybackTime() {
            if (mVodPlayer != null) {
                mVodPlayer.getCurrentPlaybackTime();
            }
            return 0;
        }
        ...
        public void seekTo(int position) {
            if (mPlayerViewCallback != null) {
                mPlayerViewCallback.onPlayTime(position);
                oldtime = position;
            }
            if (mCurrentPlayType == SuperPlayerConst.PLAYTYPE_VOD) {
                if (mVodPlayer != null) {
        ...
        public void onDanmuku(boolean on) {
            ...
            mPlayerViewCallback.onDanMuCheck(on);
        }
...
//        popupWindow.setFocusable(true);
    ...
    public void onPlayEvent(TXVodPlayer player, int event, Bundle param) {
        ...
        if (event == TXLiveConstants.PLAY_EVT_VOD_PLAY_PREPARED) { //视频播放开始
            ...
            if (onPlayEndListener != null) onPlayEndListener.onPlayEnd();
        } else if (event == TXLiveConstants.PLAY_EVT_PLAY_PROGRESS) {
            int progress = param.getInt(TXLiveConstants.EVT_PLAY_PROGRESS_MS);
            int duration = param.getInt(TXLiveConstants.EVT_PLAY_DURATION_MS);
//            if (!isPlay && progress <= 3000 && progress > 0) {
//                mVodController.pause();
//                isPlay = true;
//
//            } else {
...
//            }
            ...
            if (oldtime != progress / 1000 && mPlayerViewCallback != null) {
                mPlayerViewCallback.onPlayTime(progress / 1000);
                oldtime = progress / 1000;
            }
    ...
    public void onNetStatus(TXVodPlayer player, Bundle status) {
        if (!isHigth)
            if (player.getWidth() > 0 && player.getHeight() > 0) {
                isHigth = true;
                WidthAndKuan = (player.getWidth() - player.getHeight()) > 0 ? 1 : 0;
            }
...
//        }
    ...
    public interface OnSuperPlayerViewCallback {
        ...
        void onDanMuCheck(boolean check);
    }
    ...
    public interface OnSuperPlayerWidthCallback {
